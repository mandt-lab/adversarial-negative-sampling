...
class SupervisedNceModel(AbstractModel):
    ...
    def __init__(self, args, dat, rng, aux_model=None, log_file=sys.stdout):
        ...
        if aux_model is None or not args.initialize_to_inverse_aux:
            ...
            raise "NCE does not need special initialization"
        ...
        if aux_model is not None:
            ...
            self.log_normalizer_bias = 10 * self.log_normalizer_bias_var
        ...
        with tf.variable_scope('evaluation'):
            ...
            target_indices = self.minibatch_htr + dat.range_e * tf.range(
                tf.shape(self.minibatch_htr)[0])

            target_scores = {
                subset: tf.expand_dims(
                    tf.gather(tf.reshape(evaluation_scores[subset], (-1,)), target_indices), 1)
                for subset in ['valid', 'test']}
        ...
        with tf.variable_scope('log_likelihood'):
            ...
            if aux_model is None:
                ...
                neg_log_likelihood = (
                    (1.0 / args.neg_samples) * tf.reduce_sum(
                        tf.nn.softplus(scores_neg - lls_neg - log_normalizer))
                    + tf.reduce_sum(tf.nn.softplus(-scores_pos + lls_pos + log_normalizer)))
